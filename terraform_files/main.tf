# SERVER1: with Jenkins, Maven, Docker, Ansible, Trivy
...
resource "aws_security_group" "my_security_group1" {
 ...
 # Inbound rules for allowing SSH access
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
...
# Inbound rule for Jenkins (port 8080)
 ...
 # Inbound rule for Jenkins (port 8081)
  ingress {
    from_port   = 8081
    to_port     = 8081
    protocol    = "tcp"
...
resource "aws_instance" "my_ec2_instance1" {
  ...
  # Consider EBS volume 30GB (Configure the root block device)
  ...
  # User data script to install Maven and Java on boot
  user_data = <<-EOF
    #!/bin/bash
 ...
 #STEP 3: INSTALL ADDITIONAL TOOLS USING REMOTE-EXEC PROVISIONER
