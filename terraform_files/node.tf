# SERVER2: with Docker & Kubernetes

# STEP1: CREATING A SECURITY GROUP FOR DOCKER-K8S
# Description: This security group allows necessary ports for Kubernetes operations.
...
# Inbound rule for Kubernetes API server
  ...
  # Inbound rule for Kubernetes proxy
  ingress {
    from_port   = 8001
    to_port     = 8001
    protocol    = "tcp"
  ...
  # Inbound rule for NodePort services
  ingress {
    from_port   = 30000
    to_port     = 32767
    protocol    = "tcp"
...
resource "aws_instance" "my_ec2_instance2" {
  ...
  tags = {
    Name = "NODE-SERVER"   #Tag for easy identification
  }
